	...
	arm64) # https://github.com/docker-library/mongo/issues/485#issuecomment-970864306
		...
		if ! grep -qE '^Features.* (fphp|dcpop|sha3|sm3|sm4|asimddp|sha512|sve)( .*|$)' /proc/cpuinfo; then
            ...
            if grep 'Model' /proc/cpuinfo | grep -q 'Raspberry Pi'; then
                echo
                echo 'WARNING: Using unofficial community build for Raspberry Pi which was purpose-built to overcome the micro-architecture limitations.' >&2
                echo '  see https://github.com/themattman/mongodb-raspberrypi-binaries' >&2
                echo
            fi
		fi
		;;
